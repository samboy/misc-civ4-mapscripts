##############################################################################
## File: ArabianBigIsland.py 
##
## Updated for arabian maps: Climate only changes number of rivers
##
...
## there will usually be more islands off of the mainland to explore, and 
## the mainland will be a little smaller.
##
## The map will still (usually) have a single big mainland, but now it will
## have a few smaller islands to expore with Dhows (Caravels) in the midgame
##
...
##
## 20120629: Hack to add a few more islands
##
## 20120612: 
...
## GLOBAL TUNING VARIABLES: Change these to customize the map results
	...
	seaLevel = 0 # Arab: Force high sea level
        ...
        if seaLevel == 0:
            self.landPercent = 0.47 # ARAB SEA LEVEL
        elif seaLevel == 2:
	...
	#clim = 2 # Always Arid for Arabia
        if clim == 0: # Normal/temerate
            self.DesertPercent = 0.61
            self.PlainsPercent = 0.83
	    self.iceSlope = 0 # Almost no ice
            self.RiverThreshold = 5 # Some rivers
        elif clim == 1: # Tropical
            self.DesertPercent = 0.61 
            self.PlainsPercent = 0.83 
	    self.iceSlope = 0 
            self.RiverThreshold = 2 # Lots of rivers
        elif clim == 2: # Arid
            self.DesertPercent = 0.61
            self.PlainsPercent = 0.83
	    self.iceSlope = 0 # Almost no ice
            self.RiverThreshold = 10 # Few rivers
        elif clim == 3: # Rocky
            self.DesertPercent = 0.61
            self.PlainsPercent = 0.83
	    self.iceSlope = 0 # Almost no ice
            self.RiverThreshold = 7 # Few rivers
        elif clim == 4: # Cold
            self.DesertPercent = 0.61
            self.PlainsPercent = 0.83
	    self.iceSlope = 0 # Almost no ice
            self.RiverThreshold = 3 # More rivers
	...
	patience = 2 # Big Island fixed value
	...
	self.smoothPeaks = 1 # Forced for Arabian maps
        ...
        mapRString = "Random"
        self.totestra = 0 
        if selectionID == 1: # Debug seed
            self.totestra = 123454321 # Fixed map seed
            mapRstring = "FixedSeed"
...
class HeightMap :
	...
	self.rotateMap(0.9) # LETS HAVE FUN 
...
##            self.heightMap[i] = self.heightMap[i] * self.heightMap[i]
	...
	# Lower the height map near the poles, since it looks weird in
	# Arabian maps. Also remove land near map edge (again, weird)
	# on square maps
        for x in range(mc.hmWidth):
		for y in range(mc.hmHeight):
			if mc.WrapX == False: # Square map
				# Left and right 1/16 percent get lowered
				factor = float((x + 0.0) / (mc.hmWidth + 0.0))
				factor -= 0.5
				if factor < 0:
					factor = -factor	
				factor = 0.5 - factor
				factor *= 2
				factor += PRand.random() * 0.1
				if factor > 1:
					factor = 1
				q = self.heightMap[GetHmIndex(x,y)]
				q = (q * edgewater) + (q * 
					(1.0 - edgewater) * factor)
				self.heightMap[GetHmIndex(x,y)] = q

			# Top and bottom 1/16 percent get lowered
			factor = float((y + 0.0) / (mc.hmHeight + 0.0))
			factor -= 0.5
			if factor < 0:
				factor = -factor	
			factor = 0.5 - factor
			...
			if factor > 1:
				factor = 1
			q = self.heightMap[GetHmIndex(x,y)]
			q = (q * edgewater) + (q * (1.0 - edgewater) * factor)
			self.heightMap[GetHmIndex(x,y)] = q
	# Done
	return
...
##        am.PrintAreaMap()
    ...
    def createPlotMap(self):
        ...
        for y in range(mc.height):
            for x in range(mc.width):
                ...
                elif PRand.random() < 0.3: # Arab hack
    ...
    def createTerrainMap(self):
		...
		# Simplified for Arab maps -- Sam
                elif self.rainFallMap[i] < self.desertThreshold:
                    if PRand.random() < 0.9: # 90% chance of desert
                        self.terrainMap[i] = mc.DESERT
                    else:
                        self.terrainMap[i] = mc.PLAINS
                    # Only have tundra if climate is cold
                    if clim == 4 and self.averageTempMap[i] < mc.TundraTemp:
                        self.terrainMap[i] = mc.TUNDRA
                elif self.rainFallMap[i] < self.plainsThreshold:
                    if PRand.random() < 0.7: # 70% chance of plains
                        self.terrainMap[i] = mc.PLAINS
		    elif PRand.random() < 0.7: # 10% chance of desert
                        self.terrainMap[i] = mc.DESERT
                    else:
			...
			if PRand.random() < 0.7: # 70% chance of grass
                            self.terrainMap[i] = mc.GRASS
			elif PRand.random() < 0.9: # 20% chance of plains
                            self.terrainMap[i] = mc.PLAINS
			else: # 5% chance (roughly)
			    self.terrainMap[i] = mc.DESERT
...
##        print circlePointList
        ...
        for n in range(0,len(circlePointList),2):
            ...
            self.drawCraterLine(x1,x2,cy,n,len(circlePointList))
...
##        return
    def drawCraterLine(self,x1,x2,y,q,qmax):
	factorY = float((q + 0.0) / (qmax + 0.0))
	factorY -= 0.5
	if factorY < 0:
		factorY = -factorY
	factorY *= 2
	factorY *= factorY
        ...
        for x in range(x1,x2 + 1):
	    delta = x2 + 1 - x1
	    factorX = float((x + 0.0 - x1) / (delta + 0.0))
	    factorX -= 0.5
	    if(factorX < 0):
		factorX = -factorX
	    factorX *= 2
...
##            sm.terrainMap[i] = mc.OCEAN
            #hm.heightMap[i] = 0.0
            hm.heightMap[i] *= factorY
            hm.heightMap[i] *= factorX
...
def isAdvancedMap():
	...
	return 0
def isClimateMap():
	return 1 # Arabian maps don't allow climate to be set
	
def isSeaLevelMap():
	return 0 # Sea level is fixed to high on Arab maps
...
def generatePlotTypes():
    ...
    hm.combineMaps()
    hm.rotateMap(0.9) # Once
    hm.calculateSeaLevel()
    hm.fillInLakes()
    pb.breakPangaeas()
##    hm.Erode()
##    hm.printHeightMap()
    hm.rotateMap(0.8) # Again
    hm.calculateSeaLevel() # This also again
    hm.rotateMap(0.9) # Again
    hm.addWaterBands()
##    hm.printHeightMap()
...
def createIce():
    ...
    for y in range(iceRange):
        for x in range(mc.width):
            plot = mmap.plot(x,y)
            if (plot != 0 and plot.isWater() == True and 
			PRand.random() < iceChance and mc.WrapX == True):
